...
\section{Generalized inverse and pseudoinverse}
...
% \begin{definition}{}{}
%     A square matrix $\bold{P}$ is called a projection matrix if $\bold{P}=\bold{P}^2$.
% \end{definition}
% \begin{remark}
%     Projection matrices must have a determinant of $0$ or $1$, because
%     \begin{align*}
%         \text{det}(\bold{P}) = \text{det}(\bold{P}^2) = (\text{det}(\bold{P}))^2.
%     \end{align*}
% \end{remark}

% \begin{remark}
%     The following statements explain what a projection matrix does:\\
%     (a) A projection matrix $\bold{P}\in \C^{n\times n}$ projects any vector in $\C^n$ onto its range (column space).
%     To see this, let $\bold{x}\in \C^n$. Then
%     \begin{align*}
%         \bold{Px}=\begin{pmatrix}
%             \bold{p}_1&\dots&\bold{p}_n
%         \end{pmatrix}
%         \begin{pmatrix}
%             x_1\\
%             \vdots\\
%             x_n
%         \end{pmatrix}=\sum x_i\bold{p}_i\in\text{Col}(\bold{P}) \equiv \text{Range}(\bold{P}).
%     \end{align*}
%     (b) A projection matrix keeps all points from its range in their original places. 
%     To see this, let $\bold{v}\in\text{Range}(\bold{P})$. Then
%     \begin{align*}
%         \bold{Pv}=\bold{P(Px)} = \bold{P}^2\bold{x} = \bold{P}\bold{x}=\bold{v}.
%     \end{align*}
% \end{remark}
